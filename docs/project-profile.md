...
Workspace…*); `/agento agento-init`'s report reminds you. `doctor` cannot detect
this: the hook payload carries the working directory but no list of workspace
folders, so keep the folder added by hand. Managed sessions need no such step: they
open as a two-folder `<kind>-<id>.code-workspace` holding the product half and its
companion half.
