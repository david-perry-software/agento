...
8. Commit the scaffold on a `changes/agento-init` branch (never on the default
   ...
   default branch is protected server-side, the exact next command (`/agento start-session`
   then `/agento new-feature <description>`), and — if the plugin is not yet registered —
