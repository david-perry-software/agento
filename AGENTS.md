...
- `extension/` — the VS Code dashboard, launcher, and router over the CLI;
  `extension/cli/` is a generated byte copy of `scripts/`, so edit `scripts/` and
  then run `cd extension && npm run copy-cli`.
...
- Extension install: `cd extension && npm ci`
- Extension build: `cd extension && npm run build`
- Extension unit tests: `cd extension && npm run test:unit`
- Extension activation test: `cd extension && npm run test:electron`
